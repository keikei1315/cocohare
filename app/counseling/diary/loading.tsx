  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: '#FFF9F5', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: '52px', backgroundColor: '#fff', borderBottom: '1px solid #EDE5DC', flexShrink: 0, display: 'flex', alignItems: 'center', padding: '0 16px', gap: '12px' }}>
        ...
        <div className="sk" style={{ flex: 1, height: '16px', borderRadius: '6px', backgroundColor: '#EDE5DC', maxWidth: '120px', margin: '0 auto' }} />
      </div>
      <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto' }}>
        {[...Array(5)].map((_, i) => (
          <div key={i} className="sk" style={{ height: '72px', borderRadius: '14px', backgroundColor: '#EDE5DC' }} />
        ))}
      </div>
    </div>
  )
